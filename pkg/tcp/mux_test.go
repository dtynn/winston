...
func TestDefaultListener(t *testing.T) {
	...
	go func(ln net.Listener) {
		...
		if _, err := io.CopyN(&buf, conn, int64(len(msg)+1)); err != nil {
			errC <- err
			return
		} else if got := buf.Bytes()[1:]; !bytes.Equal(msg, got) {
			errC <- fmt.Errorf("message mismatch:\n\nexp=%x\n\ngot=%x", msg, got)
			return
		}
	...
	// Write message to TCP listener and read OK response.
	conn, err := Dial("tcp", lnd.Addr().String(), 5)
	if err != nil {
		t.Fatal(err)
	} else if _, err = conn.Write(msg); err != nil {
		t.Fatal(err)
	...
	// check error before we read response
	for err := range errC {
		if err != nil {
			t.Fatal(err)
		}
